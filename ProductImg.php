 ...
 */
    ...
    $AllProduct = $ths->GetData($ths->query("select `product_id`, `title`, `model`, `confirm_price`, `confirm_count`, `img_id` from `product` where `display`=1 and `deleted`=0 and `confirm_price`=1 and `confirm_count`=1 order by `product_id` asc;"));
...
$allimg = $ths->GetData($ths->query("select `code`, `code2`, `prop`, `count_info`, `price_off`, `product_id` from `product_price` where `product_id` in (".implode(',', $ProdIDs).") and `confirm`=1 order by `product_id` desc;"));
