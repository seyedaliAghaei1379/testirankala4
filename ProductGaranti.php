 ...
 */
    ...
    $AllProduct = $ths->GetData($ths->query("select `product_id`, `title`, `model`, `confirm_price`, `confirm_count` from `product` where `display`=1 and `deleted`=0 and `confirm_price`=1 and `confirm_count`=1 and `garanti_id`=0 order by `product_id` asc;"));
