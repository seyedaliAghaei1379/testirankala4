 ...
 */
    ...
    $AllProduct = $ths->GetData($ths->query("select `product_id`, `title`, `model`, `confirm_price`, `confirm_count`, `rel_id` from `product` where `display`=1 and `deleted`=0  order by `product_id` asc;"));
...
<?php
        ...
        foreach($AllCode as $ap){
            ...
            #if($ProdInfo[$ap->product_id]['confirm_price'] && $ProdInfo[$ap->product_id]['confirm_count'])
